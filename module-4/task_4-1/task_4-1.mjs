...
deposite(aAmount, aCurrencyType = CurrencyTypes.NOK){
    ...
    withdraw(aAmount, aCurrencyType = CurrencyTypes.NOK){
        ...
        const newAmount = aAmount / exchange;
        this.#balance -= newAmount;
        const den = this.#currency.denmonation;
        const name = aCurrencyType.name;
        printOut("withdraw of" + aAmount+", new balace is " + this.#balance.toFixed(2) + den);  
...
setCurrencyType(aType){
            ...
            printOut("The currency has change from"+ this.#currency.name +" to" + aType.name);
           const exchange = this.#currencyConvert(aType); 
            this.#currency= aType;
            this.#balance *= exchange;
        printOut( " New balance is" + this.#balance + this.#currency.denomination);
        }
  }
...
//print all These types on a single comman- separated line:
// Get all object value:
