...
const SpriteInfoList = {
 ...
 export const background= new TBackground(spcvs, SpriteInfoList);
 ...
 export function resetObstaclePassed(){
...
// Sjekk om spillet er over og vis game over screen
if (EGameStatus.state === EGameStatus.gameOver) {
  menu.showGameOver(menu.getScore());
  EGameStatus.state = EGameStatus.idle;  // Prevent multiple calls
}
let eaten = -1;
...
function setDayNight(aEvent){ 
  ...
  menu.setDayNight(isDayMode);
  // TODO: Update background and obstacles day/night mode
} // end of setDayNight
